...
class GlobDataset_MASK(Dataset):
    def __init__(self, root, img_size, img_glob='*.png',
                 data_portion=(), random_data_on_portion=True,
                 vit_norm=False, random_flip=False, vit_input_resolution=448):
        super().__init__()
        if isinstance(root, str) or not hasattr(root, '__iter__'):
            root = [root]
            img_glob = [img_glob]
        if not all(hasattr(sublist, '__iter__') for sublist in
                   data_portion) or data_portion == ():  # if not iterable or empty
            data_portion = [data_portion]
        self.root = root
        self.img_size = img_size
        self.episodes = []
    ...
    def __getitem__(self, i):
        example = {}
        image = Image.open(self.episodes[i])
        mask_loc = self.episodes[i].replace('features', 'masks').replace('png', 'npy')
        if not image.mode == "RGB":
            image = image.convert("RGB")
        if self.random_flip:
            if random.random() > 0.5:
        ...
        example["mask"] = mask.unsqueeze(0)
        return example


if __name__ == "__main__":
    dataset = GlobDataset_MASK(
        root="/home/lr-2002/code/ABC-s/dataset/billiards_balls_n2m10r30size128/training",
        img_size=256,
        img_glob="**/*.png",
        data_portion=(0.0, 0.9)
    )
    for i, d in enumerate(dataset):
        print(i, d["pixel_values"].shape)
        if i == 10:
            break